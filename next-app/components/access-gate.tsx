...
export function AccessGate({ resource, children }: { resource: Resource; children: React.ReactNode }) {
  ...
  if (!canAccess(session, resource)) {
    ...
    return <Card className="mx-auto max-w-xl p-8 text-center"><CardContent><div className="font-display text-xl font-bold text-slate-950">Bạn không có quyền truy cập</div><p className="mt-2 text-sm text-slate-500">Vai trò hiện tại không được phép mở khu vực này.</p><Button className="mt-5" onClick={() => router.push(homePath)}>Về trang chính</Button></CardContent></Card>;
  }
  return children;
}
