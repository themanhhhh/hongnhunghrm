...
const recruitmentTabs: WorkspaceTab[] = [
  {
    ...
    fields: [date("effective_date", "Ngày áp dụng", true), text("department_id", "Mã bộ phận", "dept-hr", true), text("creator_name", "Người lập", undefined, true), number("target_headcount", "Tổng định biên", true), number("max_capacity", "Sức chứa tối đa", true), number("budget", "Ngân sách tuyển dụng"), select("status", "Trạng thái", [{ value: "Tạo phiếu", label: "Tạo phiếu" }, { value: "Đang duyệt", label: "Đang duyệt" }, { value: "Đã hoàn thiện", label: "Đã hoàn thiện" }, { value: "Từ chối", label: "Từ chối" }]), json("details", "Chi tiết theo vị trí (JSON)", '[{"position_id":"pos-hr-emp","target_headcount":2,"resignation_count":0,"maternity_count":0,"note":""}]'), json("budget_details", "Phân rã ngân sách (JSON)", '[{"cost_type":"Đăng tin tuyển dụng","source":"TopCV","estimated_cost":5000000}]'), area("description", "Mô tả")],
  },
  {
  ...
  },
  {
    ...
    fields: [text("request_code", "Mã yêu cầu"), date("created_date", "Ngày lập", true), text("department_id", "Mã bộ phận", "dept-...", true), text("position_id", "Mã vị trí", "pos-...", true), text("requested_by", "Mã người lập", "emp-...", true), number("quantity", "Số lượng cần tuyển", true), date("expected_date", "Ngày cần người", true), select("priority", "Mức ưu tiên", [{ value: "LOW", label: "Thấp" }, { value: "MEDIUM", label: "Trung bình" }, { value: "HIGH", label: "Cao" }]), select("is_outside_headcount", "Định biên", [{ value: "0", label: "Trong định biên" }, { value: "1", label: "Ngoài định biên" }]), area("reason", "Lý do cần tuyển", undefined, 2), area("note", "Ghi chú nội bộ", undefined, 2)],
  },
  {
    ...
    fields: [text("candidate_code", "Mã ứng viên"), text("full_name", "Họ và tên", undefined, true, 2), select("gender", "Giới tính", [{ value: "Nam", label: "Nam" }, { value: "Nữ", label: "Nữ" }]), date("date_of_birth", "Ngày sinh"), text("citizen_id", "CCCD"), text("phone", "Điện thoại", undefined, true), text("email", "Email", undefined, true), text("address", "Địa chỉ", undefined, false, 2), select("education_level", "Trình độ", [{ value: "Cử nhân", label: "Cử nhân" }, { value: "Thạc sĩ", label: "Thạc sĩ" }, { value: "Cao đẳng", label: "Cao đẳng" }]), text("education_school", "Trường đào tạo"), text("major", "Ngành đào tạo"), text("experience", "Kinh nghiệm", undefined, false, 2), text("recruitment_plan_id", "Mã kế hoạch"), text("position_id", "Mã vị trí"), text("source", "Nguồn tuyển dụng"), text("referrer", "Người giới thiệu"), date("received_date", "Ngày tiếp nhận"), select("status", "Trạng thái", [{ value: "Đã tiếp nhận hồ sơ", label: "Đã tiếp nhận hồ sơ" }, { value: "S2: Phỏng vấn", label: "Phỏng vấn" }, { value: "S5: Trúng tuyển", label: "Trúng tuyển" }, { value: "HIRED", label: "Đã chuyển nhân viên" }, { value: "S7: Loại", label: "Loại" }]), area("note", "Ghi chú"), json("attachments_json", "Tài liệu đính kèm (JSON)", '[{"name":"CV.pdf","url":""}]')],
  },
  {
    id: "screenings", label: "Sơ loại", endpoint: "/recruitment/pre-screenings", idField: "pre_screening_id",
    columns: [{ key: "screening_code", label: "Mã phiếu" }, { key: "candidate_name", label: "Ứng viên" }, { key: "position_name", label: "Vị trí" }, { key: "level_score", label: "Mức phù hợp" }, { key: "screening_result", label: "Kết quả" }, { key: "screening_date", label: "Ngày sơ loại" }],
    fields: [text("candidate_id", "Mã ứng viên", "cand-...", true), date("received_date", "Ngày nhận hồ sơ"), text("culture_level", "Trình độ văn hóa"), text("education_level", "Trình độ chuyên môn"), text("education_school", "Trường đào tạo"), text("position_id", "Mã vị trí"), text("department_id", "Mã bộ phận"), date("screening_date", "Ngày sơ loại", true), number("level_score", "Mức độ phù hợp (0-10)", true), select("screening_result", "Kết quả", [{ value: "ĐẠT", label: "Đạt" }, { value: "KHÔNG ĐẠT", label: "Không đạt" }]), area("comment", "Ghi chú đánh giá"), json("criteria", "Tiêu chí sơ loại (JSON)", '[{"criteria_type":"Năng lực chuyên môn","candidate_value":"","is_passed":true,"note":""}]')],
  },
  {
  ...
  },
  {
    ...
    fields: [date("evaluation_date", "Ngày đánh giá", true), text("schedule_id", "Mã lịch"), text("candidate_id", "Mã ứng viên", undefined, true), number("duration_minutes", "Thời lượng (phút)"), number("level_score", "Điểm phỏng vấn (0-10)", true), select("overall_result", "Kết quả", [{ value: "ĐẠT", label: "Đạt" }, { value: "KHÔNG ĐẠT", label: "Không đạt" }]), area("overall_comment", "Nhận xét chung", undefined, 2), json("script", "Kịch bản hỏi đáp (JSON)", '[{"question":"","expectation":"","answer":""}]'), json("criteria", "Tiêu chí đánh giá (JSON)", '[{"criteria_type":"Năng lực chuyên môn","candidate_value":"","is_passed":true,"note":""}]')],
  },
  {
...
const peopleTabs: WorkspaceTab[] = [
  ...
  {
    id: "quotas", label: "Định biên nhân sự", endpoint: "/hr/quotas", idField: "quota_id", approve: true,
    columns: [{ key: "quota_code", label: "Số phiếu" }, { key: "department_name", label: "Bộ phận" }, { key: "target_headcount", label: "Định biên" }, { key: "current_headcount", label: "Hiện tại" }, { key: "status", label: "Trạng thái" }],
    fields: [date("effective_date", "Ngày áp dụng", true), text("department_id", "Mã bộ phận", undefined, true), text("creator_name", "Người lập", undefined, true), number("target_headcount", "Tổng định biên", true), number("max_capacity", "Sức chứa tối đa", true), number("budget", "Ngân sách"), select("status", "Trạng thái", [{ value: "Tạo phiếu", label: "Tạo phiếu" }, { value: "Đang duyệt", label: "Đang duyệt" }, { value: "Đã hoàn thiện", label: "Đã hoàn thiện" }, { value: "Từ chối", label: "Từ chối" }]), json("details", "Chi tiết theo vị trí (JSON)", '[{"position_id":"pos-...","target_headcount":1}]'), json("budget_details", "Phân rã ngân sách (JSON)", '[{"cost_type":"Đăng tin tuyển dụng","source":"TopCV","estimated_cost":5000000}]'), area("description", "Mô tả")],
  },
  {
  ...
  },
  {
    ...
    fields: [text("contract_no", "Số hợp đồng"), date("contract_date", "Ngày ký", true), text("signer_id", "Mã người ký"), text("signer_name", "Người ký"), text("signer_position", "Chức vụ người ký"), text("employee_id", "Mã nhân viên", undefined, true), text("employee_position", "Vị trí nhân viên"), text("contract_type", "Loại hợp đồng", undefined, true), date("start_date", "Ngày bắt đầu", true), date("end_date", "Ngày kết thúc"), date("probation_from_date", "Bắt đầu thử việc"), date("probation_to_date", "Kết thúc thử việc"), text("salary_scale", "Thang lương"), text("salary_grade", "Bậc lương"), number("base_salary", "Lương cơ sở"), number("social_insurance_salary", "Lương đóng BHXH"), json("allowance_details", "Phụ cấp (JSON)", '[{"allowance_type":"Ăn trưa","amount":500000}]'), area("job_description", "Mô tả công việc"), area("note", "Ghi chú")],
  },
  {
  ...
  },
  {
    ...
    columns: [{ key: "application_code", label: "Mã đơn" }, { key: "employee_name", label: "Nhân viên" }, { key: "desired_resign_date", label: "Ngày nghỉ dự kiến" }, { key: "reason", label: "Lý do" }, { key: "status", label: "Trạng thái" }],
    fields: [text("employee_id", "Mã nhân viên", undefined, true), date("desired_resign_date", "Ngày nghỉ dự kiến", true), area("reason", "Lý do"), area("handover_notes", "Nội dung bàn giao")],
  },
  {
    id: "resignation-decisions", label: "Quyết định nghỉ việc", endpoint: "/hr/resignation-decisions", idField: "decision_id",
    columns: [{ key: "decision_number", label: "Số quyết định" }, { key: "employee_name", label: "Nhân viên" }, { key: "official_resign_date", label: "Ngày nghỉ chính thức" }, { key: "handover_status", label: "Bàn giao" }, { key: "status", label: "Trạng thái" }],
    fields: [text("application_id", "Mã đơn nghỉ việc"), text("employee_id", "Mã nhân viên", undefined, true), date("official_resign_date", "Ngày nghỉ chính thức", true), select("handover_status", "Bàn giao", [{ value: "COMPLETED", label: "Đã hoàn tất" }, { value: "PENDING", label: "Chưa hoàn tất" }]), text("signed_by", "Người ký"), area("reason", "Lý do")],
  },
  {
    id: "work-history", label: "Quá trình công tác", endpoint: "/hr/work-history", idField: "work_history_id",
    columns: [{ key: "employee_code", label: "Mã NV" }, { key: "employee_name", label: "Nhân viên" }, { key: "department_name", label: "Bộ phận" }, { key: "position_name", label: "Vị trí" }, { key: "decision_type", label: "Loại biến động" }, { key: "effective_date", label: "Ngày hiệu lực" }],
    fields: [text("employee_id", "Mã nhân viên", undefined, true), text("department_id", "Mã bộ phận"), text("position_id", "Mã vị trí"), select("decision_type", "Loại biến động", [{ value: "Tuyển mới", label: "Tuyển mới" }, { value: "Điều chuyển", label: "Điều chuyển" }, { value: "Bổ nhiệm", label: "Bổ nhiệm" }, { value: "Miễn nhiệm", label: "Miễn nhiệm" }]), date("effective_date", "Ngày hiệu lực", true), area("reason", "Lý do"), area("note", "Ghi chú")],
  },
];
...
const rewardTabs: WorkspaceTab[] = [
  ...
  {
    id: "proposals", label: "Đề xuất thưởng phạt", endpoint: "/reward-discipline/proposals", idField: "proposal_id",
    ...
    fields: [select("record_type", "Loại đề xuất", [{ value: "KHEN_THUONG", label: "Khen thưởng" }, { value: "KY_LUAT", label: "Kỷ luật" }], true), text("employee_id", "Mã nhân viên", undefined, true), number("proposed_amount", "Số tiền"), text("proposed_by", "Người đề xuất", undefined, true), area("reason", "Lý do / nội dung", undefined, 2)],
  },
  {
    ...
    fields: [select("decision_type", "Loại quyết định", [{ value: "KHEN_THUONG", label: "Khen thưởng" }, { value: "KY_LUAT", label: "Kỷ luật" }], true), text("employee_id", "Mã nhân viên", undefined, true), date("decision_date", "Ngày ban hành", true), date("effective_date", "Ngày hiệu lực"), text("decision_by", "Người ký"), text("reason", "Lý do", undefined, true, 2), area("content", "Nội dung quyết định", undefined, 2)],
  },
  {
