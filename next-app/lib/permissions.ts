...
export const DEMO_USERS: Array<Session & { password: string }> = [
  ...
  { id: "u-khoi", name: "Lê Hoàng Nam", username: "DUNGNX", password: "123456", role: "Trưởng Khối", department: "Khối Công nghệ", employeeId: "emp-kcn-01" },
...
export function canAccess(session: Session | null, resource: Resource, action: Action = "view") {
  ...
  if (resource === "rewards" && action === "edit") {
    return ["Administrator", "HR Staff", "Ban Giám Đốc", "Trưởng Khối", "Trưởng Phòng"].includes(session.role);
  }
